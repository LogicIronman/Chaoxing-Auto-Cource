...
如果不想运行 Python/打包 exe，可以直接使用油猴脚本：

1. 安装 Tampermonkey / 篡改猴浏览器扩展。
2. 新建脚本，把 `chaoxing-auto.user.js` 的内容复制进去并保存。
3. 打开学习通课程章节页。
4. 页面右上角会出现“超星自动观看”面板。
5. 点击“扫描章节”，确认识别到未完成章节后点击“开始”。

油猴版会在页面内完成：

- 扫描 `.chapter_item` 中未完成章节。
- 自动点击章节。
- 在视频 iframe 中静音、设置 2 倍速并播放。
- 监听 `/multimedia/log/a/` 接口响应，收到 `isPassed: true` 后进入下一章节。

油猴版依赖当前页面 DOM。如果学校定制页面导致章节不使用 `.chapter_item`，需要按实际页面结构调整脚本选择器。
...
| `chaoxing-auto.user.js` | Tampermonkey 油猴脚本版 |
